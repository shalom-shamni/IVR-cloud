...
# פונקציות המודולים הישנות - מעודכנות
...
def handle_show_benefits(call_id: str):
    """הצגת זכויות"""
        ...
        if customer:
            ...
            if details:
                ...
                return jsonify({
                    ...
                    "files": [
                        {
                            "text": f"על בסיס הנתונים שלך, אתה זכאי למענק עבודה בסך {benefits['work_benefit']:.0f} שקל ולדמי לידה בסך {benefits['birth_benefit']:.0f} שקל. סה\"כ {benefits['total_benefit']:.0f} שקל. לחץ 1 לפרטים נוספים או 0 לחזרה לתפריט הראשי.",
                            "activatedKeys": "1,0"
                        }
                    ]
                })
...
class PBXHandler:
    ...
    def process_main_menu_choice(self, call_id: str, choice: str) -> Dict:
        ...
        if choice == '1':
            return self.handle_create_receipt()
        elif choice == '2':
            return self.handle_cancel_receipt()
        elif choice == '3':
            return self.handle_update_personal_details()
        elif choice == '4':
            return self.handle_show_benefits(call_id)
        elif choice == '5':
            return self.handle_leave_message()
        elif choice == '6':
            return self.handle_annual_report()
        elif choice == '0':
            return self.show_main_menu()
        else:
            return jsonify({
    ...
    def process_receipt_description(self, call_id: str, description: str) -> Dict:
        """טיפול בתיאור הקבלה ויצירתה ב-iCount"""
        call_data = self.current_calls.get(call_id, {})
        ...
        if icount_result['status']:
            ...
            return jsonify({
                ...
                "files": [
                    {
                        "text": f"שגיאה ביצירת הקבלה: {icount_result.get('message', 'שגיאה לא ידועה')}. לחץ 1 לנסות שוב או 0 לתפריט הראשי.",
                        "activatedKeys": "1,0"
                    }
                ]
            })
    ...
    def process_cancel_receipt(self, call_id: str, receipt_num: str) -> Dict:
        """טיפול בביטול קבלה"""
        # כאן נחפש את הקבלה במאגר הנתונים ונבטל אותה ב-iCount
        # לצורך הדוגמה, נחזיר תגובה כללית
    ...
    def process_children_count(self, call_id: str, num_children: str) -> Dict:
        ...
        try:
            ...
            if children_count == 0:
                # אין ילדים - מעבר לשאלה על מקומות עבודה
                return self.ask_spouse_workplaces(call_id, 1)
            else:
                # יש ילדים - שאלה על שנת לידה של הילד הראשון
                return jsonify({
                    "type": "getDTMF",
    ...
    def process_child_birth_year(self, call_id: str, input_name: str, birth_year: str) -> Dict:
        ...
        try:
            ...
            if current_child < total_children:
                ...
                # סיימנו עם כל הילדים - מעבר לשאלה על מקומות עבודה
                self.current_calls[call_id] = call_data
                return self.ask_spouse_workplaces(call_id, 1)
    ...
    def process_spouse_workplaces(self, call_id: str, input_name: str, workplaces: str) -> Dict:
        ...
        try:
            ...
            if input_name == 'spouse1_workplaces':
                # שאלה על בן/בת זוג שני
                return self.ask_spouse_workplaces(call_id, 2)
            else:
                # סיימנו - שמירה במאגר נתונים
                phone_number = call_data.get('PBXphone')
                customer = self.db.get_customer_by_phone(phone_number)
...
@app.route('/pbx/menu/<menu_choice>', methods=['GET'])
def handle_menu_choice(menu_choice):
    """טיפול בבחירות מהתפריט"""
    try:
        choice = request.args.get('mainMenu')  # השם שהגדרנו ב-name
        ...
        if choice == '1':
            ...
            return jsonify({"error": "בחירה לא חוקית"}), 400
            
    except Exception as e:
        logger.error(f"שגיאה בטיפול בבחירה: {str(e)}")
