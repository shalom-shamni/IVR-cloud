...
def handle_new_customer() -> Dict:
    return {
        ...
        "extensionChange": "",
        "files": [{
            "text": "שלום וברוך הבא בפעם השלוש מאות אלף ואחת... נראה שאין לך עדיין מנוי במערכת שלנו. לחץ 1 להצטרפות למערכת, או לחץ 2 לחזרה לתפריט הקודם.",
            "activatedKeys": "1,2"
        }]
    }
...
def handle_pbx_request():
    ...
    try:
        ...
        # רשימת עדיפויות מעודכנת (גבוה→נמוך)
        PRIORITY = [
            # שלבי רישום
            'registrationSuccess',
            'registrationFail', 
...
def handle_menu_choice(menu_name):
    try:
        ...
        if value is None:
            for k in [
                'registrationSuccess', 'registrationFail', 'newCustomerID',
